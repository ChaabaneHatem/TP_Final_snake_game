...
void cadre(int cd, int ld, int cf, int lf, int couleur)
{
	...
	gotoXY(cd, ld);
	cout << (char)220;  //coin supérieur gauche
	for (int i = cd + 1; i < cf; i++)
		cout << (char)220; //ligne horizontale du haut
	cout << (char)220;  //coin supérieur droit
	for (int i = ld + 1; i < lf; i++)
	{
		gotoXY(cd, i);
		cout << (char)219;  //ligne verticale gaughe
		gotoXY(cf, i);
		cout << (char)219;  //ligne verticale droite
	}
	gotoXY(cd, lf);
	cout << (char)223;  //coin inférieur gauche
	for (int i = cd + 1; i < cf; i++)
		cout << (char)223;  //ligne horizontale inférieure
	cout << (char)223;  //coin inférieur droit
}
